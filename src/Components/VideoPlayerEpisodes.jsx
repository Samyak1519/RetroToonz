...
export default function VideoPlayerEpisodes({
    ...
    const getThumb = (ep) =>
        posterDesktop ? normalizePath(posterDesktop) : ep?.thumbnail ? normalizePath(ep.thumbnail) : defaultPoster;
                        ...
                        {season.episodes.map((ep) => {
                            ...
                            return (
                                <article
                                    ...
                                    onClick={() => onSelectEpisode?.(ep.episodeId ?? ep.episodeNumber ?? ep.id)}
                                    tabIndex={0}
                                    onKeyDown={(e) => {
                                        if (e.key === "Enter" || e.key === " ") onSelectEpisode?.(ep.episodeId ?? ep.episodeNumber ?? ep.id);
                                    }}
                                ...
                                >
                                    <div className="relative rounded-md overflow-hidden shadow-[0_6px_18px_rgba(0,0,0,0.25)]">
                                        {/* Poster area */}
                                        <div style={{ paddingTop: "56.25%" }} className="relative bg-zinc-800">
                                            <img
                                                src={thumb}
